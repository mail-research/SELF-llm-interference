...
def olmo2_dtensor_weight_loader(actor_weights: Dict, vllm_model: nn.Module) -> nn.Module:
    stacked_params_mapping = [
        # (param_name, shard_name, shard_id)
        ("qkv_proj", "q_proj", "q"),
        ("qkv_proj", "k_proj", "k"),
    ...
    for name, loaded_weight in actor_weights:
        for param_name, weight_name, shard_id in stacked_params_mapping:
            if weight_name not in name:
                continue
            name = name.replace(weight_name, param_name)
...
__MODEL_DTENSOR_WEIGHT_LOADER_REGISTRY__ = {
    ...
    "Olmo2ForCausalLM": olmo2_dtensor_weight_loader,
