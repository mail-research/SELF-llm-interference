...
class RMDataset(Dataset):
    ...
    def _pad_to_length(self, input_ids, attention_mask):
        ...
        if curr_length < self.max_length:
            input_ids = torch.cat((input_ids, self.tokenizer.pad_token_id * torch.ones(size=(self.max_length - curr_length,), dtype=input_ids.dtype)), dim=-1)
    ...
    def __getitem__(self, item):
        ...
        chosen_response_length = chosen_input_ids.size(0)
        rejected_response_length = rejected_input_ids.size(0)
        ...
        rejected_position_ids = compute_position_id_with_mask(rejected_attention_mask)

        chosen_loss_mask = chosen_attention_mask.clone()
        rejected_loss_mask = rejected_attention_mask.clone()

        if prompt_length > 1:
            # mask out prompt for SFT.
            chosen_loss_mask[: min(prompt_length, chosen_loss_mask.size(0)) - 1] = 0
            rejected_loss_mask[: min(prompt_length, chosen_loss_mask.size(0)) - 1] = 0
        # mask out the last token in response
        chosen_loss_mask[min(prompt_length + chosen_response_length, chosen_loss_mask.size(0)) - 1] = 0
        rejected_loss_mask[min(prompt_length + rejected_response_length, rejected_loss_mask.size(0)) - 1] = 0
