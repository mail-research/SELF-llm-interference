...
def compute_outcome(token_level_rewards: torch.Tensor, response_mask: torch.Tensor):
    scores = token_level_rewards.sum(dim=-1)
    with torch.no_grad():
        scores = scores.unsqueeze(-1) * response_mask
    return scores, scores
...
def compute_psr_nsr_outcome_advantage(
    ...
    # """
    scores = token_level_rewards.sum(dim=-1)
    correct_idx = scores == 1 
    incorrect_idx = scores != 1

    with torch.no_grad():
        # scores = (scores == 1.0).float()
        advantages = scores.clone()
        advantages[correct_idx] = 0.1
        advantages[incorrect_idx] = -1.0
        advantages = advantages.unsqueeze(-1) * response_mask
        
    return advantages, advantages

# def compute_psr_nsr_outcome_advantage(token_level_rewards: torch.Tensor, response_mask: torch.Tensor):
#     """
#     Compute advantage for PSR, NSR, and W-REINFORCE.
#     Args:
#         token_level_rewards: `(torch.Tensor)`
...
def compute_value_cls_loss(vpreds, returns, response_mask, cliprange_value):
    ...
    token_loss = torch.nn.functional.binary_cross_entropy_with_logits(input=vpreds, target=returns, reduction='none')
    token_loss = verl_F.masked_mean(token_loss, response_mask)
    return token_loss
